...
const ProductReelMfgAll = (props: ProductReelProps) => {
    ...
    updatedProducts?.forEach(item => {
        if (item.manufacturer == query.product_category && !uniqueCategories.includes(item.product_category)) {
            uniqueCategories.push(item.product_category);
        }
    });
                    ...
                    return <>
                        <div className="md:flex md:items-center md:justify-between mb-4">
                            <div className="max-w-2xl px-4 lg:max-w-4xl lg:px-0">
                                <p className="mt-4 text-xl ui-sm:text-2xl font-bold leading-tight text-gray-900">Category: {category}</p>
                            </div>
                                ...
                                <div className='w-full grid grid-cols-2 gap-x-4 gap-y-10 sm:gap-x-6 md:grid-cols-4 md:gap-y-10 lg:gap-x-8'>
                                    {
                                        map_products.map((product, i) =>
                                            product?.manufacturer === queryProductCategory && product?.product_category === category ? (
                                                <ProductListing
                                                    key={`product-${i}`}
                                                    product={product}
                                                    index={i}
...
export default ProductReelMfgAll;